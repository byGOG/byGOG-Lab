...
(function () {
  function start() {
    ...
    const periodMs = (n => (Number.isFinite(n) && n > 0 ? n : 4000))(
      Number(el.getAttribute('data-float-ms'))
    );
    ...
    function busy() {
      return document.activeElement === input || (input && !!input.value);
    }

    function frame(ts) {
      raf = 0;
      if (!running || reduce) {
        root.style.setProperty('--search-bob', '0px');
        return;
      }
      if (busy()) {
        root.style.setProperty('--search-bob', '0px');
