...
(function () {
  ...
  function start() {
    ...
    const cfg = {
      phrases: phrases && phrases.length ? phrases : ['Ara...', 'Steam, Rufus, Git', 'Ctrl+K'],
      defaultText: ds.phDefault || 'Ara...',
    ...
    try {
      if (cfg.playOnce && localStorage.getItem(cfg.onceKey) === '1') {
        return;
      }
    } catch {}
    ...
    try {
      ...
      if (reduceMotion) {
        ...
        input.addEventListener('input', () => {
          if (input.value) setPlaceholder('');
        });
        input.addEventListener('blur', () => {
          if (!input.value) setPlaceholder(cfg.defaultText);
        });
        return;
      }
    } catch {}
    ...
    function tick() {
      ...
      if (cfg.playOnce && finishedOneLoop && idx === 0 && dir === 1 && pos === 0) {
        try {
          localStorage.setItem(cfg.onceKey, '1');
        } catch {}
        setPlaceholder(cfg.defaultText);
        return; // end
      }
