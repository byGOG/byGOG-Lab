...
(function () {
  ...
  function main() {
    const els = Array.from(
      document.querySelectorAll('.site-header[data-parallax], .search-container[data-parallax]')
    );
    ...
    function apply(ts) {
      ...
      if (reduce) {
        try {
          root.style.setProperty('--search-parallax', '0px');
          root.style.setProperty('--header-parallax', '0px');
        } catch {}
    ...
    function onScroll() {
      ...
      if (delta !== 0) {
        for (const t of targets) {
          t.offset = clamp(t.offset + delta * t.sens, -t.maxAbs, t.maxAbs);
        }
        schedule();
      }
    }
