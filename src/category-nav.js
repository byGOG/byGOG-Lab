...
export function initCategoryNav() {
  ...
  const scrollToSection = (entry, opts = {}) => {
    ...
    try {
      document.dispatchEvent(
        new CustomEvent('category-nav-select', {
  ...
  if (scEmbed) {
    ...
    scIcon.innerHTML =
      '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M1 18V10.5a.5.5 0 0 1 1 0V18a.5.5 0 0 1-1 0zm3 1V9a.5.5 0 0 1 1 0v10a.5.5 0 0 1-1 0zm3 0V7a.5.5 0 0 1 1 0v12a.5.5 0 0 1-1 0zm3 0V5.5a.5.5 0 0 1 1 0V19a.5.5 0 0 1-1 0zm3 0V8a.5.5 0 0 1 1 0v11a.5.5 0 0 1-1 0zm3-1V9a.5.5 0 0 1 1 0v9c0 1.5-1 2-2 2h-1a.5.5 0 0 1 0-1h1c.55 0 1-.22 1-1zm3-1V11a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2h-.5a.5.5 0 0 1 0-1h.5a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1 .5.5 0 0 1-.5-.5V10c0-2.5-1.5-4-3.5-4a.5.5 0 0 1 0-1c2.5 0 4 2 4 5v1z"/></svg>';
    ...
    scBtn.onclick = () => {
      ...
      if (clickScrollingTimer) clearTimeout(clickScrollingTimer);
      const off = navOffset();
      const top = scEmbed.getBoundingClientRect().top + window.scrollY - off;
      window.scrollTo({ top, behavior: 'instant' });
      clickScrollingTimer = setTimeout(() => {
        clickScrolling = false;
      }, 100);
  ...
  cats.forEach(h2 => {
    ...
    const populateSubItems = targetCard => {
      ...
      subHeadings.forEach(h3 => {
        ...
        const subIconSvg = getSubcategoryIcon(h3.textContent);
        if (subIconSvg) {
          const subIcon = document.createElement('span');
          subIcon.className = 'nav-sub-icon';
          subIcon.innerHTML = subIconSvg;
          subBtn.appendChild(subIcon);
        }

        const subText = document.createElement('span');
        subText.textContent = h3.textContent;
        subBtn.appendChild(subText);

        subBtn.setAttribute('aria-label', t('nav.goToSubcategory', { name: h3.textContent }));
        subBtn.onclick = () => {
