...
minifyCSS();
console.log(`  CSS minification: ${Date.now() - cssStart}ms`);
...
async function minifyJS() {
  ...
  console.log(`  JS minification: ${Date.now() - jsStart}ms`);
}
  ...
  cssFiles.forEach(file => {
    ...
    try {
      ...
      if (csso) {
        ...
        console.log(
          `Minified ${file}: ${originalSize} -> ${minifiedSize} bytes (${savings}% smaller)`
        );
      } else {
        // No csso, just copy
...
targets.forEach(src => {
  ...
  manifest[path.relative(rootDir, src).replace(/\\/g, '/')] = path
    .relative(rootDir, out)
    .replace(/\\/g, '/');
  ...
  files.forEach(f => {
    if (re.test(f)) {
      ...
      if (path.relative(distDir, keepPath) !== f) {
        try {
          fs.unlinkSync(full);
          console.log('Removed old hashed', full);
        } catch {}
      }
    }
  });
...
try {
  if (manifest['dist/styles.css']) {
    cleanOldHashed('styles', 'css', path.join(rootDir, manifest['dist/styles.css']));
  }
  if (manifest['dist/fab.css']) {
    cleanOldHashed('fab', 'css', path.join(rootDir, manifest['dist/fab.css']));
  }
  if (manifest['dist/renderLinks.js']) {
    cleanOldHashed('renderLinks', 'js', path.join(rootDir, manifest['dist/renderLinks.js']));
  }
} catch {}
...
console.log(`\nBuild completed in ${Date.now() - buildStart}ms`);
