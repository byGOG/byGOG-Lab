...
function printSummary(files, label) {
  ...
  console.log(
    `  ${colors.bright}${label.padEnd(30)}${colors.reset} ` +
...
function analyze() {
  ...
  console.log(
    `  ${colors.green}✓${colors.reset} Dist toplam:   ${formatBytes(grandTotal.size)} (gzip: ${formatBytes(grandTotal.gzip)}, br: ${formatBytes(grandTotal.brotli)})`
  );
  console.log(
    `  ${colors.blue}ℹ${colors.reset} Assets CSS:    ${formatBytes(assetsSummary.totalSize)}`
  );
  console.log(
    `  ${colors.blue}ℹ${colors.reset} Source JS:     ${formatBytes(srcSummary.totalSize)}`
  );
  ...
  if (largeFiles.length) {
    warnings.push(
      `  ${colors.yellow}•${colors.reset} ${largeFiles.length} dosya 100KB üzerinde - kod bölme düşünün`
    );
  }
