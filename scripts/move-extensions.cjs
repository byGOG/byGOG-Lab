...
const [srcIdx, srcSub] = findSub(
  internet,
  s => normalize(s.title).includes('taray') && normalize(s.title).includes('eklenti')
);
const [dstIdx, dstSub] = findSub(
  internet,
  s => normalize(s.title).includes('eklenti') && normalize(s.title).includes('güven')
);
...
for (const l of srcSub.links) {
  if (TARGET_URLS.has(l.url)) moved.push(l);
  else kept.push(l);
}
srcSub.links = kept;
...
console.log(
  'Moved %d item(s) into Eklenti/Güvenlik and removed from Tarayıcı/Eklenti.',
  moved.length
);
